...
try { // On essaie de faire des choses
    if (isset($_GET['action'])) {
        ...
        elseif ($_GET['action'] == 'addPost') {
            var_dump($_FILES["image"]); // Image stockée ici
            if (isset($_POST['submit'])) {
                if (!empty($_POST['author']) && !empty($_POST['title']) && !empty($_POST['content'])) { // ICI
                    addPost($_POST['author'], $_POST['title'], $_POST['content'], $_POST['image'], new DateTime()); // ICI
                }
                else {
                    // Autre exception
